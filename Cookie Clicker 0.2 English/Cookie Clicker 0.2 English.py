...
def newgame():
    ...
    if os.path.isfile("save.csv"):
        if askokcancel(title="Warning",message="Are you sure you want to create a new file? The old one will be deleted"):  
            os.remove('save.csv')
...
play=False
Avant_Jeu=Tk()
Labelle=Label(Avant_Jeu,text="Welcome to Cookie Clicker Python !",font='Arial 25')
Labelle.pack()
sous_labelle=Label(Avant_Jeu,text="Version "+str(version),font='Arial 10')
sous_labelle.pack()
newpartie=Button(Avant_Jeu,text="New Game",font="Arial 15",command=newgame)
newpartie.pack()
continuepartie=Button(Avant_Jeu,text="Continue to play",font="Arial 15",command=continuer)
...
def augm_clic():
    ...
    if nb_cookie>=prix:
        ...
        prix_augm="Cost: "+str(prix)
        text_craft="Make"+str(nb_craft)+" cookie(s)"
        crafter.config(text=text_craft,command=craft,font="Arial 40")
        prix_augm_craft.config(text=prix_augm)
    else:
        a_afficher="You don't have enough cookies! You are "+str(prix-nb_cookie)+" cookies short"
...
def achat_boulangerie():
    ...
    if nb_cookie>=prix_boulanger:
        ...
        text_prix_boulanger="Cost :"+str(prix_boulanger)
        affiche_prix_boulanger.config(text=text_prix_boulanger)
        text_nb_boulangerie="You currently have "+str(boulangerie)+" bakeries"
        nb_boulangerie.config(text=text_nb_boulangerie)
    else:
        a_afficher="You don't have enough cookies! You are "+str(prix_boulanger-nb_cookie)+" cookies short"
        add_console(a_afficher)
...
def achat_amelio_boulangerie():
    ...
    if nb_cookie>=prix_amelio_boulanger:
        ...
        text_prix_amelio_boulanger="Cost :"+str(prix_amelio_boulanger)
        affiche_prix_amelio_boulanger.config(text=text_prix_amelio_boulanger)
        text_nb_amelio_boulangerie="Your bakeries make "+str(valeur_boulangerie)+" cookie(s) per second"
        nb_amelio_boulangerie.config(text=text_nb_amelio_boulangerie)
    else:
        a_afficher="You don't have enough cookies! You are "+str(prix_amelio_boulanger-nb_cookie)+" cookies short"
        add_console(a_afficher)
...
augm_craft=Button(Jeu, text="Multiply by 2 the power of your clicks!",command=augm_clic,font="Arial 20")
...
acheter_boulangerie=Button(Jeu,text="Purchase 1 bakery",command=achat_boulangerie,font="Arial 20")
...
acheter_amelio_boulangerie=Button(Jeu,text="Multiply by 2 the speed of your bakeries",command=achat_amelio_boulangerie,font="Arial 20")
...
info=Label(Jeu,text="Last messages from the console:",font="Arial 20")
info.grid(column=2,row=0,ipadx=20,ipady=20,sticky="EWNS")
text_console="There is nothing to say"
...
def sauvegarder():
    ...
    fichier.close()
    add_console("Backup successfully completed!")
def reinitialise():
    global nb_cookie
    global nb_craft
    global nbdefoisouilaaugmcraft
    ...
    global prix
    global prix_boulanger
    if askokcancel(title="Warning",message="Are you sure you want to reset your game? You won't be able to recover your backup"):  
        nb_craft=1 #nb de craft de cookie par clic, valeur de base 1
        nb_cookie=0 #nb de cookie, valeur de base 0
        nbdefoisouilaaugmcraft=1 #nb de fois où on a amélioré le clic, valeur de base 1
        valeur_boulangerie=1 #nb cookie par seconde pour boulangerie, valeur de base 1
        boulangerie=0 # +1 cookie par seconde, valeur de base 0
        text_nb_boulangerie=text_nb_boulangerie="You currently have "+str(boulangerie)+" bakeries"
        nb_boulangerie.config(text=text_nb_boulangerie)
        prix_boulanger=10
...
def opt_save():
    ...
    tps.set(30000)
    opt=Tk()
    Info=Label(opt,text="Option for automatic backup",font="Arial 22")
    yes=Radiobutton(opt, variable=active,text="Activate",value=True,command=act,font="Arial 30")
    no=Radiobutton(opt, variable=active,text="Disable",value=False,command=desact,font="Arial 30")
    Info2=Label(opt,text="Define the time between each automatic backup",font="Arial 20")
    Info3=Label(opt,text="If you want to activate the options, you have to click on them!",font="Arial 10")
...
def quitter_s_save():
    if askokcancel(title="Warning",message="Are you sure you want to exit without saving? Your progress will be lost since your last backup"):
        quit()

def Twitter():
    open("https://twitter.com/LafosseRomain", new=0, autoraise=True)
...
menufichier.add_command(label="Save the game",command=sauvegarder)
menufichier.add_command(label="Reset the game",command=reinitialise)
menufichier.add_command(label="Automatic backup option",command=opt_save)
menufichier.add_command(label="Exit by saving",command=quitter_par_echap)
menufichier.add_command(label="Exit without saving",command=quitter_s_save)
menuliens = Menu(menubar,tearoff=0)
menubar.add_cascade(label="Link", menu=menuliens)
menuliens.add_command(label="My French Twitter account !",command=Twitter)
menuliens.add_command(label="To my Github page !",command=Twitter)

"""This is the place to disable full screen"""

if play:
    
    Jeu.attributes('-fullscreen', True) #Fill 'True' with 'False' to disable full screen.
    
    
    Jeu.bind('<Space>',lambda e: quit())
